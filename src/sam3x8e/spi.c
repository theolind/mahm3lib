...
uint8_t spi_init(spi_reg_t *spi, const spi_settings_t *settings) {
	...
	//spi->SPI_WPMR = 0b10100110101000000100100100000000; //needs to be before writing to MR
	spi->SPI_MR |= settings->master;
	//spi->SPI_MR |= (1<<5);	//Wait Data Read Before Transfer - prevents overrun of SPI_RDR
	//spi->SPI_MR &= ~(1<<1);	//Clear bit one in MR to set field peripheral select (default)
...
void spi_write(spi_reg_t *spi, uint16_t data) {
	...
	spi->SPI_TDR = ( (0xFFFF0000 & spi->SPI_TDR) | data); //clear register section TD and then write data.
}
