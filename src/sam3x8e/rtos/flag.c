...
OS_FlagID CoCreateFlag(BOOL bAutoReset,BOOL bInitialState)
{
    ...
    for(i=0;i<FLAG_MAX_NUM;i++)
    {
        ...
        if((FlagCrl.flagActive & (1u << i)) == 0 )
        {
            FlagCrl.flagActive |= (1u << i);         /* Initialize active flag   */
...
StatusType CoDelFlag(OS_FlagID id,U8 opt)
{
    ...
    if ((pfcb->flagActive & (1u << id)) == 0) /* Flag is valid or not               */
    {
        return E_INVALID_ID;	
    }
#endif
    ...
    {
        if((pnode->waitFlags & (1u << id)) != 0) /* If no task is waiting on flags */
    	  {
            ...
            else if (opt == OPT_DEL_ANYWAY) /* Always delete the flag         */
            {
                if(pnode->waitType == OPT_WAIT_ALL)
                {
                    ...
                    {
                        pnode->waitFlags &= ~(1u << id);   /* Update waitflags   */
                    }		
                }
                else   							
                {
...
#endif
    if ((pfcb->flagRdy & (1u << id)) != 0)    /* If the required flag is set        */
    {
        OsSchedLock()
        pfcb->flagRdy &= ~((FlagCrl.resetOpt) & (1u << id)); /* Clear the flag     */
        OsSchedUnlock();
        return E_OK;
    }
...
StatusType CoWaitForSingleFlag(OS_FlagID id,U32 timeout)
{
    ...
    if ((FlagCrl.flagActive & (1u << id)) == 0 )/* Judge flag is active or not?       */
    {
        return E_INVALID_ID;            /* Flag is deactive ,return error     */
    }	
#endif
    ...
    if ((pfcb->flagRdy & (1u << id)) != 0)    /* If the required flag is set        */
    {
        pfcb->flagRdy &= ~((pfcb->resetOpt & (1u << id))); /* Clear the flag       */
        OsSchedUnlock();
    }
    else                                /* If the required flag is not set    */
    {
        ...
        if(timeout == 0)                /* If time-out is not configured      */
        {
            ...
            FlagBlock (&flagNode, (1u << id), OPT_WAIT_ONE);
            ...
            pfcb->flagRdy &= ~((1u << id) & (pfcb->resetOpt));
            OsSchedUnlock();
        }
...
StatusType CoClearFlag(OS_FlagID id)
{
    ...
    pfcb->flagRdy &= ~(1u<<id);              /* Clear the flag                 */
    return E_OK;
}
